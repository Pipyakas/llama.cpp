...
static void moe_cache_worker_main(int wid) {
    ...
    for (;;) {
        ...
        if (!cstream[job.dev]) {
            // low-priority: bulk expert copies must not delay the dispatch
            // chain's mmv + small H2D on the high-priority compute stream
            int lo = 0, hi = 0;
            cudaDeviceGetStreamPriorityRange(&lo, &hi);
            cudaStreamCreateWithPriority(&cstream[job.dev], cudaStreamNonBlocking, lo);
        }
...
static bool moe_cache_pool_alloc(int di, size_t expert_size, int wtype, size_t budget, bool paired,
    ...
    if (!d.compute_stream) {
        int lo = 0, hi = 0;
        cudaDeviceGetStreamPriorityRange(&lo, &hi);
        CUDA_CHECK(cudaStreamCreateWithPriority(&d.compute_stream, cudaStreamNonBlocking, hi));
    }
                        ...
                        if (ok) {
                            ...
                            for (int b2 = 0; b2 < 1024; b2++)
                                for (int w2 = 0; w2 < 4; w2++)
                                    bits += _mm_popcnt_u64((unsigned long long) g.hot_pair[b2][w2]);
...
static int moe_cache_plan(int di, const int32_t * ids, int n_ids, int32_t * slot_idx) {
    ...
    for (int k = 0; k < n_ids; k++) {
        ...
        // Pure LRU admission: admit EVERY miss up to capacity so the pool
        // reflects actual routing (recency is the strongest predictor of next
        // token's experts - measured 53% recall@8 at C=16, 61.5% at C=24).
        // No throttle, no backfill-sweep pollution: inserts only happen here,
        // sourced from real routing, evicting LRU. Decode-time inserts were
        // previously blamed for RAM theft, but the measured loss was the
        // backfill fill order + throttle starvation, not the inserts themselves
        // (each insert is ~1.3MB; 40 layers x ~10 inserts/token = 0.5GB/s
        // over PCIe 3.0 x4 at 4GB/s - fits in the overlap window).
        if (inserts_left <= 0) {
            d.insert_skips++;
            d.skip_budget++;
            continue;
    ...
    // pure LRU admission: no admission throttle. The pool adapts to routing.
    // (throttle at capacity would starve the LRU of new routing info -
    // measured: 1-in-8 throttle cut hit rate to 16% vs 53% pure LRU)
...
// routing trace dump (GGML_MOE_TRACE=<path>): one line per MUL_MAT_ID MoE
// node: blkN,role,tok,eid0,eid1,...  role = gate/up/down
static FILE * g_trace_f = nullptr;
static void moe_cache_trace_dump(const char * name, const int32_t * ids, int n_ids, int64_t n_tokens) {
    if (!g_trace_f) {
        const char * p = getenv("GGML_MOE_TRACE");
        if (!p) return;
        g_trace_f = fopen(p, "w");
        if (!g_trace_f) return;
    }
    const char * role = "other";
    if (strstr(name, "_gate_exps")) role = "gate";
    else if (strstr(name, "_up_exps"))   role = "up";
    else if (strstr(name, "_down_exps")) role = "down";
    int blk = -1;
    const char * p = strstr(name, "blk.");
    if (p) blk = atoi(p + 4);
    for (int64_t t = 0; t < n_tokens; t++) {
        fprintf(g_trace_f, "%d,%s,%lld", blk, role, (long long)t);
        for (int k = 0; k < n_ids; k++) {
            fprintf(g_trace_f, ",%d", ids[t * n_ids + k]);
        }
        fprintf(g_trace_f, "\n");
    }
    fflush(g_trace_f);
...
// hidden-state trace (GGML_MOE_TRACE_HIDDEN=<path>): gate node src1 activation
// row (the router input) + chosen expert ids, for training a prediction adapter
static FILE * g_trace_h_f = nullptr;
static void moe_cache_trace_hidden(const char * name, const float * act, int n_in, const int32_t * ids, int n_ids) {
    if (!strstr(name, "_gate_exps")) return;
    if (!g_trace_h_f) {
        const char * p = getenv("GGML_MOE_TRACE_HIDDEN");
        if (!p) return;
        g_trace_h_f = fopen(p, "w");
        if (!g_trace_h_f) return;
    }
    int blk = -1;
    const char * p = strstr(name, "blk.");
    if (p) blk = atoi(p + 4);
    fprintf(g_trace_h_f, "%d", blk);
    for (int i = 0; i < n_in; i++) fprintf(g_trace_h_f, ",%.6g", act[i]);
    for (int k = 0; k < n_ids; k++) fprintf(g_trace_h_f, ",%d", ids[k]);
    fprintf(g_trace_h_f, "\n");
    fflush(g_trace_h_f);
}
...
static void moe_cache_node_time(int code, int64_t us) {
    ...
    b.on_n++;
    // only judge once the cache has had time to warm (pools filled by prefill
    // backfill, inserts stopped, steady-state hit rate reached): the first
    // ~10k engaged samples are polluted by pool construction + first-touch
    // page faults and would trip the judge on a cache that is winning.
    if (b.base_n < 1000 || b.on_n < 10000) return;
    if (b.on_n % 512 != 0) return;
...
static bool moe_cache_selftest_one(int di, ggml_type wtype, int64_t n_in, int64_t n_out,
    ...
    moe_cache_device & d = g.dev[di];
    if (!d.compute_stream) {
        // high-priority stream so the dispatch chain (mmv + its H2D inputs)
        // preempts the insert workers' bulk expert copies on the same device
        int lo = 0, hi = 0;
        cudaDeviceGetStreamPriorityRange(&lo, &hi);
        CUDA_CHECK(cudaStreamCreateWithPriority(&d.compute_stream, cudaStreamNonBlocking, hi));
    }
...
void ggml_moe_cache_register(void) {
    ggml_time_init();
    // opt-in: disabled by default. Engage only when explicitly requested via
    // GGML_CUDA_MOE_CACHE=1 or a positive GGML_CUDA_MOE_CACHE_BUDGET_MB
    // (--moe-cache N). Engagement is still gated per model (expert size,
    // type allowlist, census) and a baseline-sampled bail-out disables the
    // cache if it ever measures itself losing on this workload.
    const char * v = getenv("GGML_CUDA_MOE_CACHE");
    const char * b = getenv("GGML_CUDA_MOE_CACHE_BUDGET_MB");
    g.enabled = (v && atoi(v) > 0) || (b && atoll(b) > 0);
    // routing trace works even when the cache itself is disabled
    ggml_moe_cache.trace = moe_cache_trace_dump;
    ggml_moe_cache.trace_hidden = moe_cache_trace_hidden;
    if (!g.enabled) return;
