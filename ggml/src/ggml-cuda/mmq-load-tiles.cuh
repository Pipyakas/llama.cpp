...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_load_tiles_q2_0_128(
        const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int warp_size   = ggml_cuda_get_physical_warp_size();
    constexpr int nwarps      = ggml_cuda_mmq_get_nthreads(type, J, fallback) / warp_size;
    constexpr int I           = ggml_cuda_mmq_get_I(type, J, fallback);
    ...
    constexpr int blocks_per_iter = MMQ_ITER_K / QK2_0_128;
    constexpr int threads_per_row = blocks_per_iter * QI2_0_128;
    constexpr int nrows = warp_size / threads_per_row;
    constexpr int scale_entries_per_block = QK2_0_128 / QK8_1;
    constexpr int scale_entries_per_row = blocks_per_iter * scale_entries_per_block;

    const int txi  = threadIdx.x % threads_per_row;
    const int kbx  = txi / QI2_0_128;
    const int kqsx = txi % QI2_0_128;
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nrows*nwarps) {
        ...
        const block_q2_0_128 * bxi = (const block_q2_0_128 *) x + kbx0 + i*stride + kbx;
        const int16_t         * qxi = (const int16_t *) bxi->qs + kqsx * 4;
...
#pragma unroll
    for (int i0 = 0; i0 < I; i0 += nwarps) {
        ...
        const block_q2_0_128 * bxi = (const block_q2_0_128 *) x + kbx0 + i*stride + scale_block;
