...
struct ggml_moe_cache_api {
    ...
    // Routing trace (GGML_MOE_TRACE=<path>): called from the CPU MUL_MAT_ID
    // kernel for every MoE expert node with the selected expert ids, before
    // any cache gating. name = "blk.N.ffn_*_exps.weight"; ids = top-k expert
    // ids per token (n_ids per token, n_tokens tokens).
    void (*trace)(const char * name, const int32_t * ids, int n_ids, int64_t n_tokens);

    // Router-input hidden state trace (GGML_MOE_TRACE_HIDDEN=<path>): called
    // with the src1 activation row feeding the gate_exps node (= the router
    // input) so an expert-prediction adapter can be trained offline.
    void (*trace_hidden)(const char * name, const float * act, int n_in, const int32_t * ids, int n_ids);
};
