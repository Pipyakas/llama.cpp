...
// K2 Horizon system regex: "(?:'[sS]|'[tT]|'[rR][eE]|'[vV][eE]|'[mM]|'[lL][lL]|'[dD])|[^\r\n\p{L}\p{N}]?(?:\p{L}|\p{M}|‌|‍)+|\p{N}{1,3}| ?[^\s\p{L}\p{N}]+[\r\n]*|\s*[\r\n]+|\s+(?!\S)|\s+"
// Compared to LLAMA3, letter-runs also consume combining marks (\p{M}) and ZWNJ/ZWJ (U+200C/U+200D)
static std::vector<size_t> unicode_regex_split_custom_k2_horizon(const std::string & text, const std::vector<size_t> & offsets) {
    ...
    for (auto offset : offsets) {
        ...
        // (?:\p{L}|\p{M}|‌|‍)
        auto _is_k2_word = [&] (uint32_t cpt, unicode_cpt_flags flags) -> bool {
            return flags.is_letter || flags.is_accent_mark || cpt == 0x200C || cpt == 0x200D;
        };
        ...
        for (size_t pos = offset_ini; pos < offset_end; /*pos++*/ ) {
            ...
            // regex: [^\r\n\p{L}\p{N}]?(?:\p{L}|\p{M}|‌|‍)+
            if (!(cpt == '\r' || cpt == '\n' || flags.is_number)) {
                const auto flags_next = _get_flags(pos+1);
                if (_is_k2_word(cpt, flags) || _is_k2_word(_get_cpt(pos+1), flags_next)) {  // one or more word chars
                    pos++;
                    while (_is_k2_word(_get_cpt(pos), _get_flags(pos))) {
                        pos++;
                    }
                    _add_token(pos);
                    continue;
